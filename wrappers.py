...
class CollectDataset:
  ...
  def step(self, action):
    ...
    if done:
      ...
      if self._mode == 'eval':
        self._mean_episode_score += (float(episode['reward'].astype(np.float64).sum())) * (1/10)
        self._mean_episode_success += (float(np.sum(episode['success']) >= 1.0)) * (1/10)
        # print(self._mean_episode_success)
...
#       info['episode'] = episode
#       if self._mode == 'eval':
#         self._score_list.append((float(episode['reward'].astype(np.float64).sum())))
...
#         callback(episode)
#       if (self._episode_count == 10) and (self._mode == 'eval'):
#         print(np.mean(self._score_list), np.std(self._score_list))
#         self._score_list =[]
#         # exit()
#         self._logger.scalar('mean_eval_return', self._mean_episode_score)
#         self._logger.scalar('mean_eval_success', self._mean_episode_success)
